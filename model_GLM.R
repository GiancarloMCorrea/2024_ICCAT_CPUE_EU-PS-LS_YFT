...
# GLM:
glm_mod_1 = glm(log(catch) ~ year + quarter + cluster + den_water2 + h_sunrise + capacity + follow_echo, 
                data = model_df)
summary(glm_mod_1)
...
alt_model = glm(log(catch) ~ year*quarter*cluster + den_water2 + h_sunrise + capacity, 
                data = model_df)
...
ggsave(filename = file.path(plot_folder, model_folder, 'grid_predictions_glm_1.jpg'), width = 190, height = 150, units = 'mm', dpi = 500)
...
ggsave(filename = file.path(plot_folder, model_folder, 'time_predictions_glm_1.jpg'), width = 190, height = 150, units = 'mm', dpi = 500)
...
glm_mod_2a = glm(presence ~ year*quarter + cluster + h_sunrise + capacity + den_water2, family = binomial, data = model_df)
summary(glm_mod_2a)
...
glm_mod_2b = glm(log(catch) ~ year*quarter + cluster + den_water2 + h_sunrise + capacity + follow_echo, 
...
ggplot() +  
  ...
  facet_wrap(~ yyqq, ncol = 8) +
  labs(fill = "Predicted mean CPUE") + guides(color = 'none')
ggsave(filename = file.path(plot_folder, model_folder, 'grid_predictions_glm_2.jpg'), width = 190, height = 150, units = 'mm', dpi = 500)
...
ggplot(data = PredTime, aes(x = time, y = value, color = factor(type_cpue))) +
  ...
  theme(legend.position = c(0.85, 0.15)) +
  ylab('Predicted CPUE (by quarter)') + xlab('Time')
ggsave(filename = file.path(plot_folder, model_folder, 'time_predictions_glm_2.jpg'), width = 190, height = 150, units = 'mm', dpi = 500)
